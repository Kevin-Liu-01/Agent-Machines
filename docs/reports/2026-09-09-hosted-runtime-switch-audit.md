...
Hermes has a completed hosted runtime-switch/model/tool proof. OpenClaw has a completed hosted switch and a proven 512 MiB capacity failure. Its separate 2 GiB Daytona native tool fixture succeeded with fake model replies; this is not yet a successful hosted Daytona bootstrap or real-model job. It does not establish that 1 GiB is sufficient or guarantee capacity for arbitrary workloads. The managed workspace prefix still requires a real model to follow the instruction. No further paid OpenClaw attempt should occur before the relevant fixes are tested and deployed.
